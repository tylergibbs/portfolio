...
(define (psrt->2dtris aloreftris ndpoints di c w h)
  (if (null? aloreftris) null
    (let* ((x (first aloreftris))
          (p1 (prog
              (vector-ref ndpoints (reftri-rp1 x)) c w h di))
          (p2 (prog
              (vector-ref ndpoints (reftri-rp2 x)) c w h di)) 
          (p3 (prog
              (vector-ref ndpoints (reftri-rp3 x)) c w h di))
          ;(pp(begin (print p1) (print p2) (print p3)))
           )
      (if (or (valp p1 100 100) (valp p2 100 100) (valp p3 100 100))
          (cons (2dtri p1 p2 p3 (reftri-triary x)) (psrt->2dtris (rest aloreftris) ndpoints di c w h))
          (psrt->2dtris (rest aloreftris) ndpoints di c w h)))))
...
(define (get-2dtris avob ref l p invrotax di)
 (if (= l ref) null
     ...
     (append (psrt->2dtris (ndpoly-aloreftri ndp) ndpoints di -10 100 100)
...
(define (get-bytes btstr cam avob w h)
  (let* ((axis (cam-obj-axis cam))
        ...
        ;(z (map (λ (x) (begin (print x) (newline))) trs))
        )
