...
(define (rotndpoly ndp di1 di2 deg)
  ...
  (ndpoly cp (rot-axis axi rot-mat) (vector-map (λ (x) (rot-abt-pt x cp axi rot-mat)) (ndpoly-avop ndp)) (ndpoly-aloreftri ndp) (ndpoly-rad ndp)(vector-map (λ (x) (rot-abt-pt x cp rot-mat)) (ndpoly-avobp ndp)))))
...
(define (rotndpoly-abs ndp di1 di2 deg)
  (let*((cp (ndpoly-cp ndp))
        (rot-mat (build-rot-mat di1 di2 deg (matrix-rows cp))))
  (ndpoly cp (matrix-mul rot-mat (ndpoly-axis ndp)) (vector-map (λ (x) (rot-abt-pt-abs x cp rot-mat)) (ndpoly-avop ndp)) (ndpoly-aloreftri ndp) (ndpoly-rad ndp) (vector-map (λ (x) (rot-abt-pt-abs x cp rot-mat)) (ndpoly-avobp ndp)))))
;(define pol (rotndpoly (ndpoly (l->mat '(0 0 1)) (ll->mat '((1 0 0)(0 1 0)(0 0 1))) (vector (l->mat '(0 0 1)) (l->mat '(1 0 1)) (l->mat '(0 1 1))) (list (reftri 0 1 2 (triarray 1 (vector (vector #"\0\0\0\0"))))))
 ;          0 1 (degrees->radians 45)))
;(mat->ll (ndpoly-cp pol))
;(mat->ll (ndpoly-axis pol))
...
(define (movndpoly ndp dvec amt)
  ...
  (ndpoly (matrix-add (ndpoly-cp ndp) mov) (ndpoly-axis ndp) (vector-map (λ (x) (matrix-add mov x)) (ndpoly-avop ndp)) (ndpoly-aloreftri ndp) (ndpoly-rad ndp) (vector-map (λ (x) (matrix-add mov x)) (ndpoly-avobp ndp)))))
